...
def elliptical_segment_area(h, a, b):
    """椭圆弓形面积。S = a*b*[θ - sin(θ)*cos(θ)], θ = arccos((b-h)/b)。
    通过 clip 限定 ratio ∈ [-1,1]，使公式对 h<=0（→0）与 h>=2b（→πab）
    自动给出正确结果，无需分段处理。"""
...
def circular_segment_area(h, R):
    """圆形弓形面积。S = R²*[θ - sin(θ)*cos(θ)], θ = arccos((R-h)/R)。
    R 可为数组（球冠段截面半径沿轴向变化）。"""
    h = np.asarray(h, dtype=float)
...
def small_tank_volume_shift(h, a, b, L, alpha_deg, n_seg=2000):
    """变位后储油量 (L)，沿轴向 h(x)=h - x*tan(α) 中点矩形法数值积分。"""
...
def fit_ellipse_params(data_no_in, init_oil=262.0):
    """least_squares 拟合椭圆参数 a,b,L（初始油量 262L）。"""
...
def cap_section_radius(x_local, side="left"):
    """球冠段局部坐标 x 处的截面圆半径 ρ（不超过圆柱半径 R）。
    左: ρ=√(r²-(d-x)²)；右: ρ=√(r²-(d-(h0-x))²)。"""
    if side == "left":
...
def big_tank_volume(h_cm, alpha_deg, beta_deg, n_cyl=2000, n_cap=500):
    """实际储油罐储油量 (L)。
    横向偏转: h_eff = h*cos(β)；纵向倾斜: h(x) = h_eff - x*sin(α)。
    圆柱段 2000 段 + 左右球冠段各 500 段，中点矩形法。"""
    alpha = np.deg2rad(alpha_deg)
...
def inverse_parameters(data_out, alpha0=2.0, beta0=4.0):
    """SLSQP 反演 α、β，约束 [0°,10°]。
    目标: min Σ[V(h_k)-V(h_{k+1}) - dV_exp(k)]²。"""
    h_mm = data_out[:, 0]
    V_exp = data_out[:, 1]
...
def load_problem1_data(filepath="问题A附件1：实验采集数据表.xls"):
    """读取问题一数据（4 个 Sheet）。
    返回 dict: {'no_in', 'no_out', 'shift_in', 'shift_out'}。"""
...
def generate_synthetic_data_problem1():
    """生成问题一示例数据，结构同 load_problem1_data。"""
    a, b, L = 40.5, 25.0, 240.0
    alpha = 4.1
    init_oil = 262.0

    h_mm = np.arange(0, 501, 10)
    h_cm = h_mm / 10.0
    V_no = np.array([small_tank_volume_no_shift(h, a, b, L) for h in h_cm])
    cum_in = np.clip(V_no - init_oil, 0, None)
    no_in = np.column_stack([cum_in, h_mm])

    h_mm2 = np.arange(500, -1, -10)
    h_cm2 = h_mm2 / 10.0
    V_no2 = np.array([small_tank_volume_no_shift(h, a, b, L) for h in h_cm2])
    no_out = np.column_stack([V_no2.max() - V_no2, h_mm2])

    V_sh = np.array([small_tank_volume_shift(h, a, b, L, alpha) for h in h_cm])
    shift_in = np.column_stack([np.clip(V_sh - init_oil, 0, None), h_mm])

    V_sh2 = np.array([small_tank_volume_shift(h, a, b, L, alpha) for h in h_cm2])
    shift_out = np.column_stack([V_sh2.max() - V_sh2, h_mm2])

    return {'no_in': no_in, 'no_out': no_out,
            'shift_in': shift_in, 'shift_out': shift_out}


def generate_synthetic_data_problem2():
    """生成问题二示例数据。
    列: [流水号, 油罐号, 进油量, 出油量, 显示油高(mm), 显示油量容积(L)]
    显示油量容积 = 模型值(真实参数) + 小噪声，用于演示反演能恢复参数。"""
    alpha_true, beta_true = 2.1, 4.3
    h_mm = np.arange(100, 2900, 30)
    h_cm = h_mm / 10.0
    V = np.array([big_tank_volume(h, alpha_true, beta_true) for h in h_cm])
    rng = np.random.default_rng(42)
    V_obs = np.clip(V + rng.normal(0, 5, size=V.size), 0, None)
    n = h_mm.size
    outflow = np.zeros(n)
    outflow[1:] = np.abs(np.diff(V_obs))
...
def plot_problem1(df_no, df_shift, data_dict):
    """问题一绘图：图1 罐容表对比+散点；图2 误差分析(2x1)；图3 差异对比。"""
    # ---------- 图1 ----------
...
def sensitivity_analysis(alpha_inv, beta_inv, h_fixed_mm=1500):
    """灵敏度分析：固定 β 扫描 α∈[0°,8°]，固定 α 扫描 β∈[0°,8°]。
    打印 α±1°、β±1° 时储油量变化，判断哪个参数影响更大。"""
